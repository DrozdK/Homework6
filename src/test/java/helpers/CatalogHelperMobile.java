...
public class CatalogHelperMobile extends BasePageMobile {

    public CatalogHelperMobile(WebDriver driverMobile) {
        super(driverMobile);
    }
    ...
    public void chooseCatalogItem (int id) {
        driverMobile.findElement(By.xpath("//li[@data-id='"+id+"']")).click();
    }

    public void chooseComputerSubdirectory (String name) {
        driverMobile.findElement(By.xpath("//div[text()=' "+name+" ']/..")).click();
    }

    public List<String> getCatalogItems(){
        List <String> items = new ArrayList<>();
        driverMobile.findElements(xpath("//span[@class='catalog-navigation-classifier__item-title']/span"))
    ...
    public List<String> getItemsFromComputersBlock(){
        List <String> items = new ArrayList<>();
        driverMobile.findElements(xpath("//div[@data-id='2']//div[contains(@class, 'title')]"))
                .forEach((WebElement item)-> items.add(item.getText()));
        return items;
    }
}
